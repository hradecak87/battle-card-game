...
export type MapMovementArrowCategory = 'transfer' | 'offensive' | 'incoming'
...
export type MapMovementArrow = MyMapMovementArrow | IncomingMapMovementArrow
...
export function useMapMovementArrows({
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const [{ data: myMovements, error: myMovementsError }, { data: incomingAttacks, error: incomingAttacksError }] =
        await Promise.all([getMyMovements(), getIncomingAttacksOnMyTerritories()])
      ...
      const territoryIds = Array.from(
        new Set(inTransitMine.flatMap((movement) => [movement.origin_territory_id, movement.destination_territory_id]))
      )
