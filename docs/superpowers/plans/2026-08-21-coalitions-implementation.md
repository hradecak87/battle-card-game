...
- [ ] **Step 1**: Write `0064_coalition_rpcs.sql`. Add a helper `coalition_lock(p_coalition_id uuid)` (copy `diplomacy_lock_pair`'s shape, single-arg `pg_advisory_xact_lock(hashtext(p_coalition_id::text))`). Implement, each `security definer`, `set search_path = public`, revoke from `public, anon`, grant to `authenticated`:
  ...
  - `coalition_accept_invite(p_invite_id uuid)` — look up invite, `coalition_lock`, verify caller is `invited_player_id` and status pending; then acquire `diplomacy_lock_pair` for the caller against every current member **in a fixed order** (`order by player_id`) before final validation; re-validate not-in-coalition / below-10 / not-at-war-with-any-member; on success insert membership row, mark invite `accepted`, cancel all other pending invites/requests for this caller (any coalition), delete any `non_aggression` relations rows between the caller and every current member, insert `coalition_member_joined` world_event. **Correction: on any re-validation failure, simply `raise exception` with the specific message — do not attempt to also mark the invite `cancelled` in the same call.** A single Postgres function call is one statement/(sub)transaction: an exception aborts the whole call and rolls back everything since entry, including any status update made earlier in that same call, so "cancel it, then still raise" is not achievable in one round trip. This matches the existing `diplomacy_accept_peace` pattern (a failed accept leaves the offer/invite row untouched, still `pending`) — the invite simply remains available to retry, or the invited player/inviter can separately call `coalition_reject_invite`/`coalition_cancel_invite` if it's now permanently stale.
