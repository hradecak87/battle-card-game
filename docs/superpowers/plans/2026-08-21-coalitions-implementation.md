...
- [ ] **Step 1**: Write `0064_coalition_rpcs.sql`. Add a helper `coalition_lock(p_coalition_id uuid)` (copy `diplomacy_lock_pair`'s shape, single-arg `pg_advisory_xact_lock(hashtext(p_coalition_id::text))`). Implement, each `security definer`, `set search_path = public`, revoke from `public, anon`, grant to `authenticated`:
  ...
  - `coalition_declare_peace(p_target_id uuid)` — verify caller is leader. This migration also `create or replace`s the existing `diplomacy_propose_peace` RPC to extract its body (below the `diplomacy_require_player()` call) into a new private helper `_diplomacy_propose_peace_core(p_initiator uuid, p_target_id uuid, p_kind text, p_offered_card_ids uuid[], p_offered_territory_id integer) returns uuid`, with `diplomacy_propose_peace` becoming a thin wrapper (`v_caller := diplomacy_require_player(); return _diplomacy_propose_peace_core(v_caller, p_target_id, p_kind, p_offered_card_ids, p_offered_territory_id);`) — this preserves 100% of the existing lock/duplicate/validation/insert behavior (diff the extracted body against the live `0046` definition to confirm no logic changed, same discipline as Task 4's core-fetch step). `coalition_declare_peace` then loops every current member currently at `war` with the target and calls `_diplomacy_propose_peace_core(member_id, p_target_id, 'white_peace', '{}'::uuid[], null)` for each, skipping any pair that already has a pending offer (the core's own duplicate-offer check already handles this — catch and swallow only that specific exception per member, re-raising any other exception). Insert one `coalition_peace_signed` world_event summarizing which members had a peace offer proposed (this event name denotes "coalition-initiated peace proposal", not final signature — actual acceptance remains per-member and asynchronous, per the spec).
