...
## Latest update — 2026-08-20j (NPC contiguous expansion, live in 0048)
...
rules. Merged into `main` and **applied to the live Supabase project**
2026-08-19 (fixed a syntax bug found during live application: Postgres
`ALTER FUNCTION` does not support `IF EXISTS`, unlike `ALTER TABLE`/`DROP
FUNCTION` — removed it from the two rename statements). 11 boost card
templates seeded live. Jest 420/420, `tsc` clean, build clean, both before
and after the fix.
...
| 2 | Trading offers: show card thumbnails + tap-to-zoom detail (like on map territories) | 3 | 6 | **done** (`a6299b0`, parallel agent) | `TradeOfferList` now shows compact `TradingCard` thumbnails for offered/requested cards, sharing one `CardZoomOverlay`; offer row changed from `<button>` to a `role="button"` `div` so the zoom button can nest inside without invalid HTML. |
...
| 6 | Allow selecting attacking troops from multiple owned territories at once (dropdown multi-check); total ETA = slowest/farthest | 6 | 5 | **done, applied to live DB 2026-08-19** | Fixed 2026-08-19c — modal now supports multi-origin multi-check selection, ETA uses max contingent travel time, and SQL migration 0025 (renumbered from 0024 to avoid a parallel-agent collision with #28) preserves per-card origin metadata for recall / non-capturing retreat splits |
...
| 15 | Dynamic growth of rare/epic/legendary card supply as player count grows | 4 | 3 | **done, formula-only** (applied to live DB 2026-08-19) | Confirmed scope with user: display-formula update only, no new supply-cap enforcement (nothing in the codebase enforces `total_supply` at all — daily/level-up rewards and `admin_grant_card` only ever grant common/uncommon). `supabase/migrations/0023_dynamic_card_supply.sql`: new `card_templates.base_total_supply` column preserves the original static values as a growth baseline; `_recompute_card_supply()` sets `total_supply = base_total_supply + floor(player_count / divisor)` (divisor: rare 2, epic 5, legend 20 — rare grows fastest, legend slowest, preserving relative scarcity); recomputed automatically via a new statement-level `on_player_created_recompute_supply` trigger on `players` insert (no cron infra exists yet, so registration is the natural hook) plus a one-off backfill recompute at the end of the migration. No client code changes needed — `total_supply` was already surfaced read-only in `collection`/`exchange` pages. |
| 16 | Limit on number of territories a player can be actively claiming at once | 3 | 5 | **done, cap = 5** (applied to live DB 2026-08-19) | Confirmed limit with user: 5 concurrent claims. Investigation found the app's real empty-territory-claim path is `declare_attack` → (troop travel) → `resolve_due_movements()`'s empty/unclaimed/no-NPC-garrison branch (which sets `claim_locked_by`) — the standalone `start_claim` RPC is never called from any component/lib code (dead/legacy path), so the cap had to go in `declare_attack`, not `start_claim`. `supabase/migrations/0022_claim_limit.sql` redefines `declare_attack` (adds a pre-lock + post-lock check, mirroring the existing 32-territory-cap double-check pattern: when the target is currently empty/unclaimed/ungarrisoned — i.e. exactly the condition that will convert to a claim on arrival — reject if the caller already has `>= 5` territories with `claim_locked_by = caller and owner_id is null`) and patches `start_claim` identically for consistency. Deliberately NOT re-verified inside `resolve_due_movements()` at arrival time (that function resolves one shared batch across all players in a single transaction; raising there would abort unrelated players' resolutions too — same trade-off already accepted for the ownership cap). No client changes needed: `DeclareAttackModal` already passes RPC `error.message` straight through to the UI. |
...
| 19 | Ability to abandon/give up a territory (becomes unclaimed again) | 3 | 4 | **done** (migration applied 2026-08-19) | `supabase/migrations/0021_abandon_territory.sql`: new `abandon_territory(p_territory_id)` RPC — validates caller owns the (non-home) territory, blocks if there's an unresolved battle or an incoming in-transit movement, auto-starts a normal transfer of any stationed cards back to the caller's home territory (same formula as `start_transfer`), then clears `owner_id` (castle/village structures left standing). Client: `abandonTerritory()` in `lib/territories/api.ts`; `GarrisonModal` shows a "Vzdát se území" button (owner, non-home only) with an inline warning/confirm step so players can redirect troops via "Přesunout vojska" first if they'd rather not send them home. Applied directly to the live Supabase project via `pg.Client`/`SUPABASE_DB_URL`. |
...
| 22 | Ability to surrender mid-battle with whatever side currently holds | 5 | 5 | **done** (confirmed live via RPC probe) | Design: `docs/superpowers/specs/2026-08-19-battle-surrender-design.md`. Only available while `battle.status = 'active'` (mid-combat) — `awaiting_ready` already has its own well-defined no-show timeout, out of scope here. `supabase/migrations/0019_battle_surrender.sql`: `_finalize_battle` (redefined from `0011_claim_xp.sql`) gains a `p_defender_surrendered boolean default false` param — existing 2-arg call sites unaffected. New `surrender_battle(p_battle_id)` RPC: attacker surrenders → defender wins, attacker's remaining cards return to origin over the full original duration (existing behavior, unchanged); defender surrenders → attacker wins/captures (same capture rules as any win), defender's remaining cards flee to their **nearest other owned territory** (Chebyshev distance, home included as a candidate) instead of always home — new surrender-only behavior; the existing home-only walkover path (`awaiting_ready` no-show) is untouched. Winner still gets the standard 50 XP + 1% structure-card reward. Client: `surrenderBattle()` in `lib/battles/api.ts`; `BattleScreen.tsx` shows a "Vzdát se" button (participants only, `active` only) with an inline confirm/cancel step (no native `window.confirm`, matches existing preview-confirm UX pattern). 18/18 `BattleScreen` tests + 80 related battle tests pass, `tsc` clean. Not yet applied to the live DB. |
| 23 | Show attacker if defender is sending reinforcements; lock out reinforcements (and recall in-transit ones) once the battle arena is ready | 7 | 6 | **done** (confirmed live via RPC probe) | Design: `docs/superpowers/specs/2026-08-19-reinforcement-lock-recall-design.md`. Lock starts at the exact moment the attacker's troops arrive and a `battles` row is created (in `resolve_due_movements()`), not at `declare_attack` time. `supabase/migrations/0018_reinforcement_lock_and_recall.sql`: `start_transfer()` now rejects reinforcing a territory with an unresolved battle (`'cannot reinforce a territory with an unresolved battle'`); `resolve_due_movements()` auto-recalls any other in-transit reinforcements heading to a territory the instant a battle is created there (via new `_recall_movement_to_origin()` helper — converts the movement to a `transfer` back to its origin, with return-trip duration = elapsed time already traveled); new `recall_attack()` RPC lets the attacker cancel their own still-in-transit attack (rejected once arrived — see #14). Client: `getIncomingReinforcements()` in `lib/territories/api.ts` + `recallAttack()` in `lib/battles/api.ts`; `MyMovementsPanel` shows an amber warning ("Obránce posílá posily, dorazí dřív než tvá vojska!") when a reinforcement's ETA beats the attacker's own, plus a "Zrušit útok" button on in-transit attack rows. 13/13 targeted `MyMovementsPanel` tests + 88 related lib tests pass, `tsc` clean. NPC/empty territories are exempt (same `owner_id is null` caveat as #10) — not yet applied to the live DB. |
...
| 26 | Boost cards: split into territorial/defensive vs. offensive/military; opponent sees only count + rarity, not which cards (e.g. a "Rat" card that can flip an enemy unit without a combat round) | 8 | 6 | **done, applied to live DB 2026-08-19** | Full design in `docs/superpowers/specs/2026-08-19-boost-cards-design.md`; implemented via `supabase/migrations/0026_boost_cards.sql`. See latest-update section above for detail. |
...
| 30 | Coalition module: leader roles, combined armies for bigger battles | 9 | 2 | **done (all 3 phases complete)** | Phase 1 coalition core + non-aggression shipped in 2026-08-21e; phase 2 troop lending / shared forces uses temporary `card_instances.owner_id` reassignment on loan arrival plus `loaned_from_id` / `loan_return_at` metadata so existing battle/garrison logic works unchanged (manual recall, auto-expiry, forced auto-recall on coalition breakup/leave/kick); phase 3 shared visibility/intelligence shipped in 2026-08-21g via live-applied `0070_coalition_map_visibility.sql` plus ally map-arrow UI. Coalition backlog item is now fully complete end-to-end. |
