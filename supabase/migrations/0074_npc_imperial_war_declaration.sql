-- ---------------------------------------------------------------------------
-- 0074_npc_imperial_war_declaration.sql
--
-- Fixes NPC war-focus targeting so only `state = 'war'` relations count as
-- war enemies, and adds an "imperial" war-declaration path for large NPC
-- kingdoms before the normal expansion/attack pick logic runs.
-- ---------------------------------------------------------------------------

create or replace function _maybe_declare_npc_imperial_war(
  p_npc_id uuid,
...
declare
  ...
  v_npc_power numeric;
  v_candidate_id uuid;
  v_effective_roll numeric := coalesce(p_roll, random());
begin
  ...
  from territories
  where owner_id = p_npc_id;

  if v_owned_territory_count < 16 then
    return null;
  end if;

  if v_effective_roll >= 0.10 then
    return null;
  end if;

  v_npc_power := _npc_diplomacy_power(p_npc_id);

  with eligible_humans as (
    select p.id
    from players p
    where coalesce(p.is_npc, false) = false
      and p.id <> p_npc_id
      and v_npc_power >= 1.5 * _npc_diplomacy_power(p.id)
      and not exists (
        select 1
        from diplomacy_relations r
        where r.player_a_id = least(p_npc_id, p.id)
          and r.player_b_id = greatest(p_npc_id, p.id)
      )
      and not exists (
        select 1
        from coalition_members cm_npc
        join coalition_members cm_human
          on cm_human.coalition_id = cm_npc.coalition_id
         and cm_human.player_id = p.id
        join coalitions c
          on c.id = cm_npc.coalition_id
        where cm_npc.player_id = p_npc_id
  ...
  candidate_players as (
    select
      ...
      exists (
        select 1
        from territories human_t
        join territories npc_t
          on npc_t.owner_id = p_npc_id
         and abs(human_t.x - npc_t.x) + abs(human_t.y - npc_t.y) = 1
        where human_t.owner_id = h.id
      ) as is_bordering
    from eligible_humans h
  )
  select c.id
  into v_candidate_id
  from candidate_players c
  order by c.is_bordering desc, random()
  limit 1;
  ...
  perform _diplomacy_declare_war_core(p_npc_id, v_candidate_id);
...
revoke execute on function _maybe_declare_npc_imperial_war(uuid, numeric) from public, anon, authenticated;
grant execute on function _maybe_declare_npc_imperial_war(uuid, numeric) to service_role;
