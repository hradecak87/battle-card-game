-- Hotfix: resolve_due_movements() (introduced in 0068_troop_lending.sql)
-- contained a leftover/erroneous statement that inserted into a
-- nonexistent "card_xp" table when a territory claim completed. No
-- migration ever created that table, and this "card XP" mechanic was never
-- part of the troop-lending spec/plan -- it broke every claim completion in
-- production (resolve_due_movements() raised "relation card_xp does not
-- exist" and aborted, leaving claims stuck in 'occupying' forever).
--
-- This migration redefines the function with that block removed. All other
-- behavior is unchanged from 0068_troop_lending.sql.
