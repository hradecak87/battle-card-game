...
export default function MyLoansPanel({ myPlayerId, refreshKey, onNavigateToTerritory }: MyLoansPanelProps) {
  ...
  return (
    <div className="w-full rounded border border-zinc-800 p-4">
      <h2 className="mb-2 text-sm font-bold text-zinc-300">Moje půjčky</h2>
            ...
            return (
              <li key={key} className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3 text-sm text-zinc-300">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <p className="font-semibold">
                      ...
                      {onNavigateToTerritory ? (
                        <button
                          type="button"
                          onClick={() =>
                            onNavigateToTerritory(loan.destination_territory_x, loan.destination_territory_y)
                          }
                  ...
                  <button
                    type="button"
                    onClick={() => void handleRecall(loan)}
